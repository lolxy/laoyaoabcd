...
KUI.widget.Class('Menu', {
	...
	showContent: function (btn){
        ...
        if (conf.alignTop) { 
            bt = this.ui.nav.offset().top-mb;           
        }else{
            var mid = Math.max(this.ui.nav.offset().top, $(document).scrollTop() + (Math.max(18, $('html').prop('clientHeight') - bodyH) - 16)/2, btn.height() + mb - bodyH + 16);
